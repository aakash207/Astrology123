...
# House Pot Allocations for Navamsa Phase 3
house_pot_allocations = {
    'Aries': 20,
    'Taurus': 60,
    'Gemini': 40,
    'Cancer': None,  # Variable - set to Moon's initial Good Currency value
    'Leo': 30,
    'Virgo': 60,
    'Libra': 80,
    'Scorpio': 20,
...
def compute_chart(name, date_obj, time_str, lat, lon, tz_offset, max_depth):
    ...
    # Store Moon's initial Good Currency value for Cancer House Pot
    moon_initial_good_currency = 0.0
    ...
    for p in ['sun','moon','mars','mercury','jupiter','venus','saturn','rahu','ketu']:
        ...
        # Store Moon's initial Good Currency value BEFORE any modifications
        if planet_cap == 'Moon':
            moon_initial_good_currency = good_val
    ...
    # Also store navamsa house sign for each house (for Phase 3)
    navamsa_house_signs = {}
    for h in range(1, 13):
        nav_house_lon = (nav_lagna + (h - 1) * 30) % 360
        navamsa_house_signs[h] = get_sign(nav_house_lon)
    ...
    # 1. Initialization - Deep copy from Navamsa Phase 2
    navamsa_phase3_data = {}
    for p in ['Sun','Moon','Mars','Mercury','Jupiter','Venus','Saturn','Rahu','Ketu']:
        navamsa_phase3_data[p] = {
            ...
            'navp3_gained_from_pot': 0.0  # Track Good Moon gained from house pot
        }
    
    # 2. Initialize House Pots with Good Moon currency
    house_pots = {}
    for h in range(1, 13):
        sign = navamsa_house_signs[h]
        if sign == 'Cancer':
            # Cancer pot = Moon's initial Good Currency value
            house_pots[h] = moon_initial_good_currency
        else:
            house_pots[h] = house_pot_allocations.get(sign, 0)
    ...
    # Debtor Rank for Malefics (from Phase 1): Rahu -> Sun -> Saturn -> Mars -> Ketu
    malefic_debtor_rank = ['Rahu', 'Sun', 'Saturn', 'Mars', 'Ketu']
    ...
    while navp3_cycles < navp3_cycle_limit:
        ...
        # Process each house
        for h in range(1, 13):
            # Get planets in this Navamsa house
            planets_in_house = navamsa_house_planets[h]
            if not planets_in_house:
                continue
            
            # Check available pot
            pot_available = house_pots[h]
            if pot_available <= 0.001:
                continue
            
            # Classify Moon for this cycle
            ...
            for planet in planets_in_house:
                if planet in standard_malefics:
                    house_malefics.append(planet)
                elif planet == 'Moon' and moon_is_malefic:
                    house_malefics.append(planet)
            
            # Build benefic list for this house
            house_benefics = []
            for planet in planets_in_house:
                if planet in standard_benefics:
                    house_benefics.append(planet)
                elif planet == 'Moon' and not moon_is_malefic:
                    house_benefics.append(planet)
            
            # Sort Malefics by Debtor Rank with special Moon ranking
            def get_malefic_rank(planet):
                if planet == 'Moon':
                    # Calculate Moon's Bad Percentage
                    nav_volume = navamsa_phase3_data['Moon']['nav_volume']
                    if nav_volume > 0:
                        bad_pct = (moon_bad_currency / nav_volume) * 100
                    else:
                        bad_pct = 0
                    # If Bad % > 25: Rank Moon before Mars (index 3.5)
                    # Else: Rank Moon after Mars (index 4.5)
                    if bad_pct > 25:
                        return 3.5  # Before Mars (Mars is at index 3)
                    else:
                        return 4.5  # After Mars, before Ketu (Ketu is at index 4)
                else:
                    return malefic_debtor_rank.index(planet) if planet in malefic_debtor_rank else 99
            
            house_malefics.sort(key=get_malefic_rank)
            
            # Phase A: Malefic Priority
            all_malefics_cleared = True
            for planet in house_malefics:
                debt = navamsa_phase3_data[planet]['navp3_current_debt']
                if debt < -0.001:
                    all_malefics_cleared = False
                    # Planet has debt, consume from pot
                    pot_available = house_pots[h]
                    if pot_available > 0.001:
                        take = min(1.0, pot_available, abs(debt))
                        if take > 0:
                            house_pots[h] -= take
                            # Add Good Moon to inventory
                            navamsa_phase3_data[planet]['navp3_inventory']['Good Moon'] = \
                                navamsa_phase3_data[planet]['navp3_inventory'].get('Good Moon', 0.0) + take
                            # Reduce debt (make it less negative)
                            navamsa_phase3_data[planet]['navp3_current_debt'] += take
                            navamsa_phase3_data[planet]['navp3_gained_from_pot'] += take
                            navp3_something_happened = True
                elif debt >= -0.001:
                    # No debt, but still consume if pot has currency (bonus consumption)
                    pot_available = house_pots[h]
                    if pot_available > 0.001:
                        take = min(1.0, pot_available)
                        if take > 0:
                            house_pots[h] -= take
                            navamsa_phase3_data[planet]['navp3_inventory']['Good Moon'] = \
                                navamsa_phase3_data[planet]['navp3_inventory'].get('Good Moon', 0.0) + take
                            # Good currency still reduces debt (becomes more positive/less negative)
                            navamsa_phase3_data[planet]['navp3_current_debt'] += take
                            navamsa_phase3_data[planet]['navp3_gained_from_pot'] += take
                            navp3_something_happened = True
            
            # Check if all malefics have cleared debt for Phase B condition
            all_malefics_cleared = True
            for planet in house_malefics:
                if navamsa_phase3_data[planet]['navp3_current_debt'] < -0.001:
                    all_malefics_cleared = False
                    break
            
            # Phase B: Benefic Secondary (only if all malefics cleared or no malefics)
            if all_malefics_cleared or len(house_malefics) == 0:
                # Calculate Debt Percentage for benefics and sort (highest first)
                def get_benefic_debt_pct(planet):
                ...
                house_benefics.sort(key=lambda x: -get_benefic_debt_pct(x))
                
                for planet in house_benefics:
                    debt = navamsa_phase3_data[planet]['navp3_current_debt']
                    pot_available = house_pots[h]
                    if pot_available <= 0.001:
                        break
    ...
    # 4. Format Navamsa Phase 3 Output
    navamsa_phase3_rows = []
    for p in ['Sun','Moon','Mars','Mercury','Jupiter','Venus','Saturn','Rahu','Ketu']:
        d_navp3 = navamsa_phase3_data[p]
        
        # Format inventory
        inv = d_navp3['navp3_inventory']
        inv_parts = []
        own_keys = [p, f"Good {p}", f"Bad {p}"]
        if p == 'Moon': own_keys = ["Good Moon", "Bad Moon"]
        for k in own_keys:
            if k in inv and inv[k] > 0.001: inv_parts.append(f"{k}[{inv[k]:.2f}]")
        for k, v in inv.items():
            if k not in own_keys and v > 0.001: inv_parts.append(f"{k}[{v:.2f}]")
        currency_str = ", ".join(inv_parts) if inv_parts else "-"
        
        # Format debt
        debt_val = d_navp3['navp3_current_debt']
        if abs(debt_val) < 0.01:
            debt_str = "0.00"
        else:
